...
#define MATCH_PER_STREAM_OPT_INT(name, type, outvar, fmtctx, st)\
{\
    int i, ret, matches = 0;\
    SpecifierOpt *so;\
    for (i = 0; i < o->nb_ ## name; i++) {\
...
int choose_encoder(OptionsContext *o, AVFormatContext *s, OutputStream *ost)
{
    ...
    if (type == AVMEDIA_TYPE_VIDEO || type == AVMEDIA_TYPE_AUDIO || type == AVMEDIA_TYPE_SUBTITLE) {
        MATCH_PER_STREAM_OPT_INT(codec_names, str, codec_name, s, ost->st);
        if (!codec_name) {
...
OutputStream *new_output_stream(OptionsContext *o, AVFormatContext *oc, enum AVMediaType type, int source_index)
{
    ...
    if (ost->enc) {
        ...
        ost->autoscale = 1;
        MATCH_PER_STREAM_OPT_NULL(autoscale, i, ost->autoscale, oc, st);
    ...
    MATCH_PER_STREAM_OPT_NULL(muxing_queue_data_threshold, i, ost->muxing_queue_data_threshold, oc, st);
...
OutputStream *new_video_stream(OptionsContext *o, AVFormatContext *oc, int source_index)
{
    ...
    MATCH_PER_STREAM_OPT_NULL(max_frame_rates, str, max_frame_rate, oc, st);
